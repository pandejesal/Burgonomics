...
- Gates: core tsc + 38/221 green. Core commit LOCAL (diverged).
### 2026-09-11 — dead generic Firestore helpers deleted (partner)
- `useFirestore.ts` (128 lines): collection/doc hooks + arbitrary-path
  add/update/delete/get with zero consumers, no barrel, no tests. Also
  removes write footguns that bypass branch scoping. Scope clean (1 file).
- Gates: partner typecheck + 30/151 green. Partner commit LOCAL (diverged).
...
- `CategoryTabs`, `CustomizationPicker`, `MenuItemCard`,
  ...
  zero references outside barrels (two not even barrelled). Files +
  barrel lines removed. Scope clean.
- Gates: core tsc + 38/221 green. Core commit LOCAL (diverged).
...
- `PetpoojaLogsPage`: reads via gateway (mock/live per env flag — queued
  ...
  filtered rows. No fake actions. Dismissed.
- No source changes. Handoff only.
...
- `PetpoojaDashboardPage`: "Mock interface active" + 3× "Simulated" badges
  ...
  surface repository errors verbatim. Dismissed.
- No source changes. Handoff only.
### 2026-09-11 — ratings honesty review (no-op, core)
- Order ratings persist on-device with "saved on this device" copy (no
  thank-the-store theater); terminal detection uses terminal/kind/code
  triple (robust to unknown codes). Dismissed.
- No source changes. Handoff only.
...
- `DeliveryPanel` + `DeliverySlotPicker` (255 lines): zero references outside
  ...
  Files + barrel lines removed. Scope clean.
- Gates: core tsc + 38/221 green. Core commit LOCAL (diverged).
...
- `PetpoojaQueuesPage`: wrapper toasted "Triggering/Success" around
  ...
  Scope clean (1 file).
- Gates: partner typecheck + 30/151 green. Partner commit LOCAL (diverged).
### 2026-09-11 — honest settings save (partner)
- `SystemSettingsTab`: save alert() claimed persisted + broadcast to
  production workers while saving nothing (local state only). Honest
  local-only toast. Scope clean (1 file).
- Gates: partner typecheck + 30/151 green. Partner commit LOCAL (diverged).
